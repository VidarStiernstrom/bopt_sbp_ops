function [H, HI, D1, D2, DI] = bopt_ops_4(N, h, options)
    % N: Number of grid points
    % h: grid spacing
    % options: struct containing options for constructing the operator
    %          current options are: 
    %               options.stencil_type ('narrow','seminarrow','wide')
    ...
    min_inds = implementations.d2_sparsity_pattern_inds(m, order, BP, 0, 2);
    nonmin_inds = implementations.d2_sparsity_pattern_inds(m, order, BP, 1, 2);

    % narrow 5 point stencil width
    function D2 = D2_fun_narrow(c)
    ...
    %  Non-narrow 7 point stencil width
    function D2 = D2_fun_seminarrow(c)
        % Here we add variable diffusion
        C1 = sparse(diag(c));
